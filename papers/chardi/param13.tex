...
% robust affine registration of brain images.  Medical Images Analysis
% 5(2) 143-156).  
...
%\begin{eqnarray*}
%  \label{simeqs}
%  \text{TPD}(\q_l) &=&   \text{HG}(\q_l; \bsL^{(1)}, \bld{V})\\
%  \text{PD}(\q_l) &=&   \text{G}(\q_l;  \bsL^{(1)}, \bld{V})\\
%  \text{ID}(\q_l) &=&   \text{HG}(\q_l; \bsL^{(2)}, \bld{V})\\
%  \text{FD}(\q_l) &=& \frac{2}{3}\text{G}(\q_l; \bsL^{(1)}, \bld{V})
%  + \frac{1}{3} \text{HG}(\q_l; \bsL^{(1)}, R_\theta\bld{V})\\
%  \text{BD}(\q_l) &=& \frac{1}{2} \text{HG}(\q_l; \bsL^{(1)}, \bld{V})
%  + \frac{1}{2} \text{HG}(\q_l; \bsL^{(1)}, R_\theta\bld{V})
%\end{eqnarray*}
%The notation $\text{TPD}(\q_l)$ is a
%truncated prolate diffusion process, $\text{PD}(\q_l)$ is a prolate
%diffusion process, $\text{ID}(\q_l)$ is an isotropic diffusion
%process, $\text{FD}(\q_l)$ is a forking diffusion process and
%$\text{BD}(\q_l)$ is a bending diffusion process.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[!htbp]
  \begin{center}
...
% In both Figures we have the following denominations:
% $\text{TPD}(\q_l)$ is red, $\text{PD}(\q_l)$ is blue,
% $\text{ID}(\q_l)$ is maroon, $\text{FD}(\q_l)$ is black and
% $\text{BD}(\q_l)$ is yellow.  The theoretical distribution (when
% existing) under some specified null is included in some of the plots
% in green.
